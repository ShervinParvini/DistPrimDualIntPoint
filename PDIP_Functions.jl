...
@everywhere type Agent
  agentname::Int64                       # Name Agents with Integers (Usefull for debugging)
  ProcsId::Int64                         # The processor in which the Agent is supposed to live
  var::Vector{Int64}                     # Index of variables
  parent::Union{Agent,Void}              # Parent Agent
  children::Union{Vector{Agent},Void}    # Child Agents
  ChProcsId::Vector{Int64}               # The processors in which the children Agents are supposed to live
  Sep                                    # Seperator of variables w.r.t variables defined on child Agents
  Res::Vector{Int64}                     # Residual of variables w.r.t variables defined on parent Agent
  ...
  lambda::Array                          # lambda (the multiplier in Lagrangian which corresponds to the ineqaulity constraint)
  Delta_lambda::Array                    # Delta_lambda
  v::Array                               # v (the multiplier in Lagrangian which corresponds to the eqaulity constraint)
  ...
  H1::Array                              # Partitioning in the upward pass stage which needs to be stored to be used in the downwardpass stage
  H2::Array                              # Partitioning in the upward pass stage which needs to be stored to be used in the downwardpass stage
  h1::Array                              # Partitioning in the upward pass stage which needs to be stored to be used in the downwardpass stage
  h2::Array                              # Partitioning in the upward pass stage which needs to be stored to be used in the downwardpass stage
end

function setparent(N::Agent)
...
function setchildrenprocsid(N::Agent)
  if ~(N.children == nothing)
    for i=1:length(N.children)
...
function Preprocessing_EquCons(N::Agent)
...
@everywhere function init_lambda_v_x(N::Agent,x)
...
@everywhere function ResidualPDMessagePassing(N::Agent,x_loc,lambda,v,t1)
...
@everywhere function upwardpass(N::Agent,t1)
  E_RC=zeros(length(N.Res),length(N.var)); for iii=1:length(N.var); if any(N.Res.==N.var[iii]);E_RC[find(N.Res.==N.var[iii]),iii]=1;end;end
...
@everywhere function downwardpass(N::Agent,Delta_x_send)
...
function MessagePassingDirection(N::Agent,t1,n)
  upwardpass(N,t1)
  Delta_x_tot = downwardpass(N,nothing)
end

@everywhere function  find_Delta_lambda(N::Agent)
...
@everywhere function stepsize_firststage(N::Agent,Beta)
...
@everywhere function TotalResiduals(N::Agent,t1,n)
...
@everywhere function TotalResiduals_Ahead(N::Agent,t1,n,t_step)
...
function MessagePassingStepSize(N::Agent,t1,alpha,Beta,n)
...
@everywhere function find_surduality(N::Agent)
...
@everywhere function Update_lambda_v_x(N::Agent,stepsize)
